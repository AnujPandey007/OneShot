...
export default function Alert(props) {
    ...
    if(props.customAlert && props.customAlert.type==="info"){
        return (
            <div className="flex p-4 mb-4 text-sm text-blue-800 border border-blue-300 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400 dark:border-blue-800" role="alert">
                ...
                <div>
                <span className="font-medium">Please Wait...</span> {props.customAlert.message}
                </div>
            </div>
          );
    }else if(props.customAlert && props.customAlert.type==="success") {
        return (
            <div className="flex p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400 dark:border-green-800" role="alert">
                ...
                <span className="sr-only">Info</span>
                <div>
                    <span className="font-medium">Success!</span> {props.customAlert.message}
                </div>
            </div>
        );
    }else if(props.customAlert && props.customAlert.type==="danger") {
        return (
            <div className="flex p-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
                ...
                <span className="sr-only">Info</span>
                <div>
                    <span className="font-medium">Danger!</span> {props.customAlert.message}
                </div>
            </div>
        );
    }
